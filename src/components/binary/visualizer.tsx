...
import { BarItem, SearchingStep, binarySearchSteps, generateRandomArray, stateStyles } from "./binarySearch";
...
export default function Visualizer() {
	...
	const resetArray = (size: number) => {
		...
		const newSteps = binarySearchSteps(newArray, randomTarget);
		setSteps(newSteps);
	...
	return (
		<>
			...
			<div className="flex w-full flex-col gap-4" ref={containerRef}>
				<h2 className="mb-2 text-2xl font-bold">Binary Search Visualizer</h2>
				<Legend />
					...
					<div className="md:col-span-2">
						<div className="bg-card flex flex-col rounded-lg border p-4">
							...
							<div className="mt-4 flex flex-wrap justify-center gap-2 text-center">
								...
								{currentStep && currentStep.left >= 0 && currentStep.right >= 0 && !currentStep.finished && (
									<span className="bg-muted rounded-md px-3 py-1 text-sm font-medium">
										Range: [{currentStep.left}, {currentStep.right}]
									</span>
								)}
								{currentStep && currentStep.mid >= 0 && (
									<span className="bg-muted rounded-md px-3 py-1 text-sm font-medium">Mid: {currentStep.mid}</span>
								)}
							</div>
						</div>
					</div>
