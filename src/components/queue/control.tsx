...
interface ControlProps {
	onResetAction: (queueSize: number) => void;
	onStartAction: () => void;
	onNextAction: () => void;
	onPrevAction: () => void;
	onPauseAction: () => void;
		...
		<div className="bg-background flex flex-col flex-wrap gap-4 rounded-lg border p-4">
			<div className="grid w-full gap-12 gap-y-4 md:grid-cols-2">
				<div className="flex flex-col gap-2">
					<Label htmlFor="queue-size">Number of Operations: </Label>
					<ElasticSlider
						min={5}
						max={12}
						isStepped
						...
						onValueChange={(value: number) => setQueueSize(value)}
						disabled={isPlaying}
						className="py-2"
					/>
					<Button variant="outline" onClick={() => onResetAction(queueSize)} disabled={isPlaying}>
						Generate New Queue
					</Button>
				</div>
